...
public class StarWoodsSapling extends BlockSapling {

	private IIcon[] iicon = new IIcon[8];
	private int type;

	public StarWoodsSapling(int type) {
		super();
	...
	public void registerBlockIcons(IIconRegister iicon) {
		for (int i = 0; i < 8;  i ++) {
			this.iicon[i] = iicon.registerIcon(this.getTextureName() + "-" + (i + type));
		}
	}
	...
	protected boolean generateTree(World world, int x, int y, int z, Random random, int meta) {
		StarWoodsBlockCore core = new StarWoodsBlockCore();
		return new WorldGenStarWoods(((meta & 7) + type) % 16, this, core.logStarWoodsNatural[((meta & 7) + type) / 16], core.leavesStarWoods[((meta & 7) + type) / 16]).generate(world, random, x, y, z);
	}
