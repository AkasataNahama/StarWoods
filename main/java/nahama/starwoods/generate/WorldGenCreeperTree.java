...
public class WorldGenCreeperTree extends WorldGenAbstractTree {

	public WorldGenCreeperTree() {
		super(true);
	}
	...
	public boolean generate(World world, Random random, int x, int y, int z) {
		...
		if (!bottom.canSustainPlant(world, x, y - 1, z, ForgeDirection.UP, (CreeperTreeSapling)StarWoodsBlockCore.saplingCreeperTree)) {
			return false;
		}
					...
					for (int iz = z - l; iz <= z + l; iz ++) {
						...
						if (Math.abs(ix - x) != l || Math.abs(iz - z) != l || (h2 != 0 && random.nextInt(2) == 0)) {
							...
							if (block.isAir(world, ix, iy, iz) || block.isLeaves(world, ix, iy, iz)) {
								this.setBlockAndNotifyAdequately(world, ix, iy, iz, StarWoodsOreDicCore.TakumiLeaves, 0);
							}
						}
					}
				}
		...
		for (int iy = 0; iy < h; iy ++) {
			...
			if (block.isAir(world, x, y + iy, z) || block.isLeaves(world, x, y + iy, z) || block == StarWoodsOreDicCore.TakumiLeaves) {
				this.setBlockAndNotifyAdequately(world, x, y + iy, z, StarWoodsOreDicCore.TakumiWoods, 0);
			}
		}

		//最上段の1つ上に炎を生成する。
		Block block = world.getBlock(x, y + h + 1, z);
		if (block.isAir(world, x, y + h + 1, z) || block.isLeaves(world, x, y + h + 1, z) || block == StarWoodsOreDicCore.TakumiLeaves) {
			this.setBlockAndNotifyAdequately(world, x, y + h + 1, z, Blocks.fire, 0);
		}
